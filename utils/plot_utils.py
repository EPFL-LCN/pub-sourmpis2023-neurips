...
def trial_metric(
    filt_train, filt_test, filt_jaw_train, filt_jaw_test, session_info, model
):
    p_corr_train = []
    for sess in range(len(session_info[0])):
        (
            ...
            filt_data_sess_test,
            filt_data_jaw_sess,
            filt_data_jaw_sess_test,
            idx,
        ) = session_tensor(
            sess, session_info, filt_train, filt_test, filt_jaw_train, filt_jaw_test
        )
        pop_train, pop_test = model.feature_pop_avg(
            ...
            idx,
            sess,
            z_score=True,  # model.opt.z_score
        )
        if pop_train.shape[0] == 0:
            continue
        pop_train, pop_test = pop_train.T, pop_test.T
        test_trials = pop_train.shape[1]
        train_trials = pop_test.shape[1]
        p_corr_sess = []
        for i in range(10):
            ...
            idxx, indexy = linear_sum_assignment(cost.detach().cpu().numpy())
            pop_train, pop_test = pop_train[:, idxx], pop_test[:, indexy]
            p_corr_sess.append(pear_corr(pop_train.T, pop_test.T, dim=1).mean())
        p_corr_train.append(np.array(p_corr_sess).mean())
    return np.array(p_corr_train)
